...
static struct win *
win_find_by_window(xcb_window_t window)
{
	return win_hash_get(window);
}

static void
tab_update_wins(struct tab *);
...
static void
win_label_rerender(struct win *w)
{
...
static void user_focus_win(struct user *, struct win *);
...
user_save_pointer(struct user *u)
{
	...
	struct win *w = win_find_by_window(reply->child);
	if (w) {
...
user_focus_win(struct user *u, struct win *w)
{
	...
	if (MODE_MOUSE == u->mode || (MODE_RELABEL == u->mode && !w))
	...
	if (oldw) {
		...
		oldt = oldw->tab;
		win_label_rerender(oldw);
		win_update_net(oldw);
	}

	if (oldaltw && (oldaltw != w && oldaltw != oldw))
		win_label_rerender(oldaltw);
...
static void
user_feed_normal(struct user *u, xkb_keysym_t keysym,
...
static void
user_feed_command(struct user *u, xkb_keysym_t keysym,
		xcb_input_key_press_event_t const *event)
{
	if (XCB_INPUT_KEY_RELEASE == event->event_type)
		return;
		...
		xcb_input_key_press_event_t const *event)
{
	...
	case XKB_KEY_plus:
	case XKB_KEY_minus:
		user_feed_normal(u, XKB_KEY_Escape, event);
...
#if 0
	fprintf(stderr, "%s key=0x%x mods=%d,%d,%d,%d deviceid=%d sourceid=%d (root=%x)\n",
			XCB_INPUT_KEY_PRESS == event->event_type ? "DOWN" : "UP",
			keysym,
...
static void
win_label_del(struct win_label *);
...
handle_unmap_notify(xcb_unmap_notify_event_t const *event)
{
	...
	struct win *w = win_find_by_window(event->window);
	if (w)
		win_del(w);
}

static struct win *win_new(xcb_window_t window);
...
win_new(xcb_window_t window)
{
	...
	struct xdo_cookie void_cookies[] = {
	...
	struct win *parent = win_find_by_window(w->transient_for);
	if (!w->tab && parent) {
...
load_tree(struct proc *proc)
{
	...
	while (0 < getline(&line, &linesz, f)) {
		...
		struct win *w = win_find_by_window(window);
		if (!w)
			continue;
